...
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  pub_poseimu_without_cov = nh->advertise<geometry_msgs::PoseStamped>("poseimu_without_cov", 2);
  PRINT_DEBUG("Publishing: %s\n", pub_poseimu_without_cov.getTopic().c_str());
...
void ROS1Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  _nh->param<std::string>("frame_id_world", frame_id_world_, "global");
...
void ROS1Visualizer::visualize_odometry(double timestamp) {
  ...
  if (pub_odomimu.getNumSubscribers() != 0) {
    ...
    odomIinM.header.frame_id = frame_id_world_;
  ...
  trans.frame_id_ = frame_id_world_;
...
void ROS1Visualizer::publish_state() {
  ...
  poseIinM.header.frame_id = frame_id_world_;
  ...
  // Create and publish PoseStamped message (without covariance)
  geometry_msgs::PoseStamped poseIinM_without_cov;
  poseIinM_without_cov.header = poseIinM.header;
  poseIinM_without_cov.pose = poseIinM.pose.pose;
  pub_poseimu_without_cov.publish(poseIinM_without_cov);
...
void ROS1Visualizer::publish_features() {
  ...
  sensor_msgs::PointCloud2 cloud = ROSVisualizerHelper::get_ros_pointcloud(feats_msckf, frame_id_world_);
  pub_points_msckf.publish(cloud);

  // Get our  features
  std::vector<Eigen::Vector3d> feats_slam = _app->get_features_SLAM();
  sensor_msgs::PointCloud2 cloud_slam = ROSVisualizerHelper::get_ros_pointcloud(feats_slam, frame_id_world_);
  pub_points_slam.publish(cloud_slam);

  // Get our  features
  std::vector<Eigen::Vector3d> feats_aruco = _app->get_features_ARUCO();
  sensor_msgs::PointCloud2 cloud_aruco = ROSVisualizerHelper::get_ros_pointcloud(feats_aruco, frame_id_world_);
  ...
  // Get our  SIMULATION features
  std::vector<Eigen::Vector3d> feats_sim = _sim->get_map_vec();
  sensor_msgs::PointCloud2 cloud_SIM = ROSVisualizerHelper::get_ros_pointcloud(feats_sim, frame_id_world_);
...
void ROS1Visualizer::publish_groundtruth() {
  ...
  arrIMU.header.frame_id = frame_id_world_;
...
void ROS1Visualizer::publish_loopclosure_information() {
  ...
  if (pub_loop_pose.getNumSubscribers() != 0 || pub_loop_extrinsic.getNumSubscribers() != 0 ||
    ...
    odometry_pose.header.frame_id = frame_id_world_;
  ...
  if (pub_loop_point.getNumSubscribers() != 0) {
    ...
    point_cloud.header.frame_id = frame_id_world_;
