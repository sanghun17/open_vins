...
public:
   ...
   */
  void set_ground_truth_for_init(const Eigen::Vector3d &position, const Eigen::Vector4d &orientation);

    /// If we are initialized or not
  bool initialized() { return is_initialized_vio && timelastupdate != -1; }
  
  /// Check if ZUPT was successful (for odometry publishing)
  bool zupt_successful() { return did_zupt_update; }
