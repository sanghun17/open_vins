...
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  pub_poseimu_with_covariance = nh->advertise<geometry_msgs::PoseWithCovarianceStamped>("poseimu_with_covariance", 2);
  PRINT_DEBUG("Publishing: %s\n", pub_poseimu_with_covariance.getTopic().c_str());
  ...
  nh->param<std::string>("child_frame_id", child_frame_id, "imu");
...
void ROS1Visualizer::visualize() {
  ...
  // Return if we have not inited (but use our own condition instead of problematic initialized())
  if (_app->get_state() == nullptr || _app->get_state()->_timestamp <= 0) {
    // std::cout << "[DEBUG] visualize: no valid state, returning early" << std::endl;
    return;
  }
  // std::cout << "[DEBUG] visualize: proceeding with state publishing" << std::endl;
  ...
  visualize_odometry(last_visualization_timestamp);
...
void ROS1Visualizer::visualize_odometry(double timestamp) {
  // std::cout << "visualize_odometry" << std::endl;
  // Let's monitor timelastupdate and initialized() components
  // bool is_init = _app->initialized();
  // bool has_state = (_app->get_state() != nullptr);
  // bool has_timestamp = has_state && (_app->get_state()->_timestamp > 0);
  
  // Try to access timelastupdate through VioManager (we'll need to check if this is accessible)
  // std::cout << "[DEBUG] visualize_odometry: state=" << has_state << ", timestamp_ok=" << has_timestamp 
  //           << ", initialized=" << is_init << std::endl;
  
  // Check if we have a valid state instead of using problematic initialized() check
  if (_app->get_state() == nullptr || _app->get_state()->_timestamp <= 0) {
    // std::cout << "[DEBUG] visualize_odometry: no valid state, returning early" << std::endl;
    return;
  }
  // std::cout << "[DEBUG] visualize_odometry: proceeding with odometry publishing (timestamp=" << timestamp << ")" << std::endl;
  ...
  if (!_app->get_propagator()->fast_state_propagate(state, timestamp, state_plus, cov_plus)) {
    std::cout << "visualize_odometry:fast_state_propagate failed" << std::endl;
    return;
  }
  ...
  // int num_subscribers = pub_odomimu.getNumSubscribers();
  // std::cout << "[DEBUG] pub_odomimu subscribers: " << num_subscribers << std::endl;
  if (pub_odomimu.getNumSubscribers() != 0) { 
    // std::cout << "[DEBUG] Publishing odometry message" << std::endl;

    nav_msgs::Odometry odomIinM;
    odomIinM.header.stamp = ros::Time(timestamp);
    odomIinM.header.frame_id = frame_id;
    ...
    odomIinM.child_frame_id = child_frame_id;
    ...
    // std::cout << "[DEBUG] Odometry message published successfully!" << std::endl;
  } else {
    // std::cout << "[DEBUG] No subscribers for odomimu topic" << std::endl;
  }
  ...
  trans.frame_id_ = frame_id;
  trans.child_frame_id_ = child_frame_id;
  if (publish_global2imu_tf) {
    // std::cout << "visualize_odometry:publish_global2imu_tf" << std::endl;
  ...
  // std::cout << "visualize_odometry:end" << std::endl;
}
...
void ROS1Visualizer::callback_inertial(const sensor_msgs::Imu::ConstPtr &msg) {
  ...
  // Only publish odometry if system is initialized
  // We've confirmed the system is working (ZUPT messages), so let's just publish odometry
  // Skip the problematic initialized() check and use our own condition
  bool has_state = (_app->get_state() != nullptr);
  bool has_timestamp = has_state && (_app->get_state()->_timestamp > 0);
  
  if (has_state && has_timestamp) {
    // std::cout << "[DEBUG] IMU callback: Publishing odometry (bypass initialized check)" << std::endl;
  ...
  std::thread thread([&] {
    ...
    if (unique_cam_ids.size() == num_unique_cameras) {
      ...
      while (!camera_queue.empty() && camera_queue.at(0).timestamp < timestamp_imu_inC) {
        ...
        // std::cout << "[DEBUG] Before feed_measurement_camera: initialized=" << _app->initialized() << std::endl;
        _app->feed_measurement_camera(camera_queue.at(0));
        // std::cout << "[DEBUG] After feed_measurement_camera: initialized=" << _app->initialized() << std::endl;
        // std::cout << "[DEBUG] Camera update completed, timelastupdate should now be set" << std::endl;
        visualize();
...
void ROS1Visualizer::callback_monocular(const sensor_msgs::ImageConstPtr &msg0, int cam_id0) {
  // std::cout << "callback_monocular" << std::endl;
...
void ROS1Visualizer::publish_state() {
  // std::cout << "publish_state" << std::endl;
  ...
  // Create simple PoseStamped message for poseimu
  geometry_msgs::PoseStamped poseIinM_simple;
  poseIinM_simple.header.stamp = ros::Time(timestamp_inI);
  poseIinM_simple.header.seq = poses_seq_imu;
  poseIinM_simple.header.frame_id = frame_id;
  poseIinM_simple.pose.orientation.x = state->_imu->quat()(0);
  poseIinM_simple.pose.orientation.y = state->_imu->quat()(1);
  poseIinM_simple.pose.orientation.z = state->_imu->quat()(2);
  poseIinM_simple.pose.orientation.w = state->_imu->quat()(3);
  poseIinM_simple.pose.position.x = state->_imu->pos()(0);
  poseIinM_simple.pose.position.y = state->_imu->pos()(1);
  poseIinM_simple.pose.position.z = state->_imu->pos()(2);
  pub_poseimu.publish(poseIinM_simple);

  // Create PoseWithCovarianceStamped message for poseimu_with_covariance
  geometry_msgs::PoseWithCovarianceStamped poseIinM_with_cov;
  poseIinM_with_cov.header.stamp = ros::Time(timestamp_inI);
  poseIinM_with_cov.header.seq = poses_seq_imu;
  poseIinM_with_cov.header.frame_id = frame_id;
  poseIinM_with_cov.pose.pose.orientation.x = state->_imu->quat()(0);
  poseIinM_with_cov.pose.pose.orientation.y = state->_imu->quat()(1);
  poseIinM_with_cov.pose.pose.orientation.z = state->_imu->quat()(2);
  poseIinM_with_cov.pose.pose.orientation.w = state->_imu->quat()(3);
  poseIinM_with_cov.pose.pose.position.x = state->_imu->pos()(0);
  poseIinM_with_cov.pose.pose.position.y = state->_imu->pos()(1);
  poseIinM_with_cov.pose.pose.position.z = state->_imu->pos()(2);
  ...
  for (int r = 0; r < 6; r++) {
    for (int c = 0; c < 6; c++) {
      poseIinM_with_cov.pose.covariance[6 * r + c] = covariance_posori(r, c);
    }
  }
  pub_poseimu_with_covariance.publish(poseIinM_with_cov);

  //=========================================================
  //=========================================================

  // Append to our pose vector
  poses_imu.push_back(poseIinM_simple);
  ...
  arrIMU.header.frame_id = frame_id;
...
void ROS1Visualizer::publish_images() {
  // std::cout << "publish_images" << std::endl;
  // Return if we have already visualized
...
void ROS1Visualizer::publish_groundtruth() {
  ...
  poseIinM.header.frame_id = frame_id;
