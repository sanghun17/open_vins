...
public:
  ...
  static void gram_schmidt(const Eigen::Vector3d &gravity_inI, Eigen::Matrix3d &R_GtoI) {
    ...
    // FIXED INITIAL ORIENTATION: Set X-axis to point forward (North)
    // This will give consistent initial orientation instead of arbitrary
    Eigen::Vector3d desired_x_axis(1.0, 0.0, 0.0);  // Fixed X direction
    
    // Check if gravity is not parallel to desired X axis
    if (fabs(z_axis.dot(desired_x_axis)) < 0.9) {
      // Use cross product to get orthogonal X axis
      x_axis = desired_x_axis - z_axis * z_axis.dot(desired_x_axis);
      x_axis = x_axis / x_axis.norm();
    } else {
      // If gravity is too close to X axis, use Y axis instead
      Eigen::Vector3d desired_y_axis(0.0, 1.0, 0.0);
      x_axis = desired_y_axis - z_axis * z_axis.dot(desired_y_axis);
      x_axis = x_axis / x_axis.norm();
    }
