...
void VioManager::set_ground_truth_for_init(const Eigen::Vector3d &position, const Eigen::Vector4d &orientation) {
  // Only set ground truth data if we haven't initialized yet
  if (!is_initialized_vio && initializer != nullptr) {
    // Try to set ground truth data in the initializer
    // We'll use a simple approach by checking the type name
    std::string initializer_type = typeid(*initializer).name();
    if (initializer_type.find("InertialInitializer") != std::string::npos) {
      // Cast to InertialInitializer and set ground truth data
      auto inertial_init = std::dynamic_pointer_cast<ov_init::InertialInitializer>(initializer);
      if (inertial_init != nullptr) {
        inertial_init->set_ground_truth(position, orientation);
        // Only print once when data is actually set (StaticInitializer will handle the actual setting)
        // PRINT_INFO(GREEN "[VIO]: Ground truth data set in InertialInitializer!\n" RESET);
      } else {
        PRINT_WARNING(YELLOW "[VIO]: Failed to cast to InertialInitializer!\n" RESET);
      }
    } else {
      PRINT_WARNING(YELLOW "[VIO]: Initializer type not supported for ground truth: %s\n" RESET, initializer_type.c_str());
    }
  } else if (is_initialized_vio) {
    // System is already initialized, ignore ground truth data silently
    // PRINT_DEBUG("[VIO]: System already initialized, ignoring ground truth data\n");
  } else {
    PRINT_WARNING(YELLOW "[VIO]: No initializer available!\n" RESET);
  }
}
