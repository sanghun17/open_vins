...
bool InertialInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  // 디버깅 로그 추가
  PRINT_INFO(GREEN "[DEBUG] params.init_imu_thresh: %.10f\n" RESET, params.init_imu_thresh);
  PRINT_INFO(GREEN "[DEBUG] has_jerk: %d, is_still: %d\n" RESET, has_jerk, is_still);
  PRINT_INFO(GREEN "[DEBUG] wait_for_jerk: %d\n" RESET, wait_for_jerk);
...
void InertialInitializer::set_ground_truth(const Eigen::Vector3d &position, const Eigen::Vector4d &orientation) {
  // Forward ground truth data to the static initializer
  if (init_static != nullptr) {
    init_static->set_ground_truth(position, orientation);
    // Only print once when data is actually set
    // PRINT_INFO(GREEN "[init]: Ground truth data forwarded to StaticInitializer!\n" RESET);
  } else {
    PRINT_WARNING(YELLOW "[init]: StaticInitializer not available!\n" RESET);
  }
}
