/*
  ...
  */
  ...
  ROS1Visualizer::ROS1Visualizer(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
    ...
    pub_poseimu_stamped = nh->advertise<geometry_msgs::PoseStamped>("poseimu_stamped", 2);
    PRINT_DEBUG("Publishing: %s\n", pub_poseimu_stamped.getTopic().c_str());
  ...
  // Subscribe to ground truth odometry for initialization
  sub_gt_odom = nh->subscribe("/gt_odom", 1, &ROS1Visualizer::callback_gt_odom, this);
  PRINT_DEBUG("Subscribing: %s\n", sub_gt_odom.getTopic().c_str())
  ...
  void ROS1Visualizer::visualize_odometry(double timestamp) {
    ...
    // Return if we have not inited OR if no ZUPT success
    if (!_app->initialized() && !_app->zupt_successful())
      return;
    ...
    // Publish our pose stamped message if requested (without covariance)
    if (pub_poseimu_stamped.getNumSubscribers() != 0) {

      geometry_msgs::PoseStamped poseIinM;
      poseIinM.header.stamp = ros::Time(timestamp);
      poseIinM.header.frame_id = frame_id;

      // The POSE component (orientation and position)
      poseIinM.pose.orientation.x = state_plus(0);
      poseIinM.pose.orientation.y = state_plus(1);
      poseIinM.pose.orientation.z = state_plus(2);
      poseIinM.pose.orientation.w = state_plus(3);
      poseIinM.pose.position.x = state_plus(4);
      poseIinM.pose.position.y = state_plus(5);
      poseIinM.pose.position.z = state_plus(6);

      pub_poseimu_stamped.publish(poseIinM);
    }
    ...
    for (const auto &calib : state->_calib_IMUtoCAM) {
      ...
      trans_calib.frame_id_ = child_frame_id;
  ...
  void ROS1Visualizer::publish_loopclosure_information() {
    ...
    if (pub_loop_pose.getNumSubscribers() != 0 || pub_loop_extrinsic.getNumSubscribers() != 0 ||
      ...
      odometry_pose.header.frame_id = frame_id;
      ...
      odometry_calib.header.frame_id = child_frame_id;
  ...
  void ROS1Visualizer::callback_gt_odom(const nav_msgs::Odometry::ConstPtr &msg) {
    // Extract position and orientation from ground truth odometry
    ...
    // Set ground truth data for initialization
    _app->set_ground_truth_for_init(position, orientation);
    
    PRINT_DEBUG(GREEN "[GT]: Received ground truth data for initialization!\n" RESET);
    PRINT_DEBUG(GREEN "[GT]: Position: %.3f, %.3f, %.3f\n" RESET, position(0), position(1), position(2));
    PRINT_DEBUG(GREEN "[GT]: Orientation: %.3f, %.3f, %.3f, %.3f\n" RESET, 
