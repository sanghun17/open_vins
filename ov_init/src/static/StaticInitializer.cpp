...
bool StaticInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<Type>> &order,
  ...
  // OVERRIDE: Set initial values from ground truth if available
  if (has_gt_data) {
    // Use ground truth data for initialization
    imu_state.block(0, 0, 4, 1) = gt_orientation;  // Use GT orientation
    imu_state.block(4, 0, 3, 1) = gt_position;     // Use GT position
    PRINT_INFO(GREEN "[init-s]: Using ground truth for initialization!\n" RESET);
    PRINT_INFO(GREEN "[init-s]: GT Position: %.3f, %.3f, %.3f\n" RESET, 
               gt_position(0), gt_position(1), gt_position(2));
    PRINT_INFO(GREEN "[init-s]: GT Orientation: %.3f, %.3f, %.3f, %.3f\n" RESET,
               gt_orientation(0), gt_orientation(1), gt_orientation(2), gt_orientation(3));
    
    // Clear ground truth data after using it (one-time use)
    has_gt_data = false;
  ...
  // Velocity: [vx, vy, vz] (in m/s) - always zero for static initialization
  imu_state.block(7, 0, 3, 1) << 0.0, 0.0, 0.0;
...
void StaticInitializer::set_ground_truth(const Eigen::Vector3d &position, const Eigen::Vector4d &orientation) {
  // Only set ground truth data if we haven't already set it
  if (!has_gt_data) {
    gt_position = position;
    gt_orientation = orientation;
    has_gt_data = true;
    PRINT_INFO(GREEN "[init-s]: Ground truth data set for initialization!\n" RESET);
  }
