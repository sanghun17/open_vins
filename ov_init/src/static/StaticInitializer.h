...
public:
  ...
  /**
   * @brief Set ground truth data for initialization
   * @param position Ground truth position [x, y, z]
   * @param orientation Ground truth orientation [x, y, z, w] (quaternion)
   */
  void set_ground_truth(const Eigen::Vector3d &position, const Eigen::Vector4d &orientation);
...
private:
  ...
  /// Ground truth data for initialization
  Eigen::Vector3d gt_position = Eigen::Vector3d::Zero();
  Eigen::Vector4d gt_orientation = Eigen::Vector4d(0, 0, 0, 1);
  bool has_gt_data = false;
};
